...
class CombatBoard:
    ...
    def update_board(self, event):
        if event.type == KEYDOWN:
            current_position = [self._combat_objects[0].get_board_position()[0], self._combat_objects[0].get_board_position()[1]]


            if event.key == K_LEFT and current_position[0] > 0:
                self._combat_objects[0].set_board_position([current_position[0] - 1, current_position[1]])
                new_position = [current_position[0] - 1, current_position[1]]
                print('went left')

            elif event.key == K_RIGHT and current_position[0] < (self._board_dimensions[0] - 1):
                self._combat_objects[0].set_board_position([current_position[0] + 1, current_position[1]])
                new_position = [current_position[0] + 1, current_position[1]]
                print('went right')

            elif event.key == K_UP and current_position[1] > 0:
                self._combat_objects[0].set_board_position([current_position[0], current_position[1] - 1])
                new_position = [current_position[0], current_position[1] - 1]
                print('went up')

            elif event.key == K_DOWN and current_position[1] < (self._board_dimensions[1] - 1):
                self._combat_objects[0].set_board_position([current_position[0], current_position[1] + 1])
                new_position = [current_position[0], current_position[1] + 1]
                print('went right')
            ...
            self._board_tiles[current_position[0]][current_position[1]] = Tile(False)
